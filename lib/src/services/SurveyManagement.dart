...
class SurveyManagement{
  addNewAnswer({answer1, answer2, answer3, answer4, context}){
    Firestore.instance.collection('/Survey').document('Q1').setData({
      'Answer': answer1,
    });
    Firestore.instance.collection('Survey').document('Q2').setData({
      'Answer': answer2,
    });
    Firestore.instance.collection('Survey').document('Q3').setData({
      'Answer': answer3,
    });
    Firestore.instance.collection('Survey').document('Q4').setData({
      'Answer': answer4,
    }).then((value){
